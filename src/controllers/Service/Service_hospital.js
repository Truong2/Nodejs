  ...
  const list_service = await Service.aggregate([
    ...
    },
    {
      $group: {
        ...
        specialistId: { $push: "$Specialist.Specialist_Name" },
...
exports.update_service_service = async (req, res) => {
  const { service_id, serviceCost, serviceName, HosId, SpecialistId, type_service } = req.body;
  ...
  if (service) {
    await Service.findOneAndUpdate({
      ...
      .then(() => { return res.status(StatusCodes.OK).json({ message: "Cập nhật dịch vụ thành công.", statusCode: 200 }) })
      .catch((err) => { return res.status(404).json({ message: err.message, statusCode: 404 }) })
  } else {
